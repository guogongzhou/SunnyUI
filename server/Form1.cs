...
namespace caiwu
{
    public partial class Form1 : Form
    {
        ...
        public void todo(object sender, EventArgs e) {
            ...
            if (now.Minute == runMinute&& now.Second== runSecond)
                {
                LogHelper.WriteLog("开始执行");

                //long start_add_time = 1591977600;
             
                String pay_status = "2";
                DateTime today = new DateTime(now.Year, now.Month, now.Day);
                long start_add_time = DateTimeTool.getUtcTrick(today);

                long end_add_time = DateTimeTool.getUtcTrick(now) - 28800;
                ...
                //String url = "https://www.feifeigo.cn/api.php?app_key=A20DE5F4-BEA5-4E43-A5DC-AC173661F372&page_size=1000&method=dsc.order.list.get&start_add_time=1592323200&end_add_time=1592387999";
                ...
                if (rootobject.info.list.Length > 0)
                {
                    ...
                    String sql_del_Orderlist = "delete from t_orderlist where add_time>"+ start_add_time + "";
                    String sql_del_Ordergood = "delete from t_ordergoods where add_time>" + start_add_time + "";

                    String sql = "insert into t_orderlist(order_success_time,order_id,main_order_id,order_sn,user_id,order_status,shipping_status,pay_status,consignee,country,province,city,district,street,address,zipcode,tel,mobile,email,best_time,sign_building,postscript,shipping_id,shipping_name,shipping_code,shipping_type,pay_id,pay_name,how_oos,how_surplus,pack_name,card_name,card_message,inv_payee,inv_content,goods_amount,cost_amount,shipping_fee,insure_fee,pay_fee,pack_fee,card_fee,money_paid,surplus,integral,integral_money,bonus,order_amount,return_amount,from_ad,referer,add_time,confirm_time,pay_time,shipping_time,confirm_take_time,auto_delivery_time,pack_id,card_id,bonus_id,invoice_no,extension_code,extension_id,to_buyer,pay_note,agency_id,inv_type,tax,is_separate,parent_id,discount,discount_all,is_delete,is_settlement,sign_time,is_single,point_id,shipping_dateStr,supplier_id,froms,coupons,uc_id,is_zc_order,zc_goods_id,is_frozen,drp_is_separate,team_id,team_parent_id,team_user_id,team_price,chargeoff_status,invoice_type,vat_id,tax_id,is_update_sale,ru_id,main_count,rel_name,id_num,stores_name,rate_fee,main_pay,child_show,store_start_time,store_end_time,selectTime,country_name,province_name,city_name,district_name,street_name,zhandianmingcheng) values " + stringBuilder + "";
                    String sq2 = "insert into t_ordergoods(order_success_time,add_time,order_id,rec_id,user_id,cart_recid,goods_id,goods_name,goods_sn,product_id,goods_number,market_price,goods_price,goods_attr,send_number,is_real,extension_code,parent_id,is_gift,model_attr,goods_attr_id,ru_id,shopping_fee,warehouse_id,area_id,area_city,is_single,freight,tid,shipping_fee,drp_money,is_distribution,commission_rate,stages_qishu,product_sn,is_reality,is_return,is_fast,rate_price,goods_bonus,cost_price,goods_coupons,buy_drp_show,membership_card_id,membership_card_discount_price,is_received,main_count,is_comment,country_name,province_name,city_name,district_name,street_name) values " + stringBuilder_orderGoods + "";
                    int sq2_idx = sq2.LastIndexOf(",");
                    sq2 = sq2.Substring(0, sq2_idx);
                    SQLHelper.updateCgd(sql_del_Orderlist);
