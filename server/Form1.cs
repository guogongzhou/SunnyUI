...
namespace caiwu
{
    public partial class Form1 : Form
    {
        ...
        public void todo(object sender, EventArgs e) {
            LogHelper.WriteLog("计时器");

            System.DateTime time = DateTimeTool.TodayDate();

            //每天的10:50干活
            //if (time.Hour == 10 && time.Minute == 50 && time.Second == 0).
            LogHelper.WriteLog(time.ToString());
            LogHelper.WriteLog(time.Minute.ToString());
            LogHelper.WriteLog(time.Second.ToString());
            if (time.Hour == 12)
            {
                LogHelper.WriteLog("time.Hour == 12");
            }
            if ( time.Minute == 19)
            {
                LogHelper.WriteLog("时间到开始执行");
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            LogHelper.WriteLog("请求地址"+ url);
            LogHelper.WriteLog("返回数据" + rep_str);
